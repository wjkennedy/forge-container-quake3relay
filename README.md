...
You'll also need to install version `12.1.0` (or later) of the Forge CLI. This version includes all the CLI tools required to manage containers.

### Register app <a id="register-app"></a>

This app's manifest uses two environment variables (namely, `TAG` and `APP_ID`). By default, `TAG` uses the value `latest` as configured in your app's manifest. You'll need to declare a value for `APP_ID`:
